...
## 성과 및 기여도
+ 사용자 대상 선적 변경 알림 자동화로 운영 개입 90% 감소
+ 알림 센터 도입으로 통합적인 알림 관리 가능
+ 국가별 타임존 적용 및 예약 발송 기능으로 사용자 맞춤 알림 정확도 향상
+ 알림 이력 기반의 백오피스 구축으로 운영 재처리 및 감사 대응 편의성 확보
+ Azure Service Bus 기반 안정적인 메시지 큐 처리 구조 완성
+ Slack 연동을 통한 장애 실시간 감지 및 운영 대응 속도 향상
