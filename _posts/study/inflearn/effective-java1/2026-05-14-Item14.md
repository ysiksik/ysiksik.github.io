...
이번 시간은 `Comparable` 아이템의 마지막 정리 시간이다.
앞에서 `Comparable`, `Comparator`, `compareTo()` 구현 방식까지 살펴봤다면, 이번에는 그 배경에 깔린 중요한 개념들을 조금 더 깊게 정리해보자.

특히 이번 내용은 단순히 `Comparable`만의 이야기가 아니라:
...
`Integer.MIN_VALUE`는 이미 표현 가능한 최소값이다.
